...
export async function resolveTrade(tradeId: string): Promise<void> {
  ...
  // Try the in-memory price feed first (populated by the Bybit price feed).
  // If it's empty for any reason, generate a small random price movement so
  // the result is ~50/50 instead of deterministically always making BUY lose.
