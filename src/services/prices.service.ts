...
/* ════════════════════════════════════════════════════════════
   ...
   TRENDING_ORDER pins a fixed set of pairs to the top of the list,
   in this exact order, regardless of live volume. Everything else
   in CURATED_BASES fills out the remaining slots sorted by volume.
...
// Additional curated pairs (blue-chip / high-liquidity majors) that fill
// out the rest of the 100-pair list, ordered by live volume at sync time.
const CURATED_OTHER: string[] = [
  "ADA", "DOT", "LINK", "AVAX", "UNI", "MATIC", "TON", "NEAR", "APT", "ARB",
  "ICP", "FIL", "VET", "ALGO", "EGLD", "THETA", "XMR", "DASH", "XLM", "HBAR",
  "ETC", "BCH", "EOS", "XTZ", "NEO", "MANA", "SAND", "GALA", "KAVA", "ROSE",
  "ZIL", "ANKR", "CELO", "STX", "RUNE", "KSM", "WAVES", "QTUM", "OMG", "BAT",
  "MASK", "CRV", "COMP", "YFI", "SUSHI", "1INCH", "REN", "KNC", "BAL", "LDO",
  "RPL", "FXS", "GMX", "DYDX", "PYTH", "JUP", "JTO", "WLD", "SEI", "TIA",
  "STRK", "MANTA", "ALT", "PORTAL", "PIXEL", "ACE", "RON", "IMX", "APE", "GMT",
  "WOO", "SKL", "STORJ", "OCEAN", "RSR", "CTSI", "POND", "DENT", "HOT", "PEOPLE",
  "DUSK", "HIGH", "LOOM", "TWT",
];

export const CURATED_BASES: Set<string> = new Set([...TRENDING_ORDER, ...CURATED_OTHER]);

const TRENDING_RANK: Map<string, number> = new Map(TRENDING_ORDER.map((b, i) => [b, i]));

/** Sort comparator: trending pairs first (in TRENDING_ORDER), then the rest by volume desc. */
function curatedSort<T extends { baseAsset?: string; base?: string; volume: number }>(a: T, b: T): number {
  const baseA = (a.baseAsset ?? a.base)!;
  const baseB = (b.baseAsset ?? b.base)!;
  const rankA = TRENDING_RANK.has(baseA) ? TRENDING_RANK.get(baseA)! : Infinity;
  const rankB = TRENDING_RANK.has(baseB) ? TRENDING_RANK.get(baseB)! : Infinity;
