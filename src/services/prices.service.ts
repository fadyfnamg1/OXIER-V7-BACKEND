...
/**
 * Sort comparator for the markets list as shown to users: ranked purely by
 * payout percentage, highest first, since payout is now the single
 * backend-driven number that's the same for everyone. Ties broken by 24h
 * % change, then volume, just to keep the order stable rather than
 * shuffling pairs with identical payout on every request.
 */
function payoutSort<T extends { payout?: number; change?: number; volume?: number }>(a: T, b: T): number {
...
async function fetchLiveMarketsUncached(): Promise<LiveMarket[]> {
  const [exchangeInfo, tickers, futuresTickers, marketSettings] = await Promise.all([
    ...
    fetch(`${FUTURES_BASE}/ticker/24hr`, { signal: AbortSignal.timeout(10000) })
      .then((r) => (r.ok ? r.json() : []))
      .catch(() => []),
    // Single source of truth for payout — the same value trade.service.ts
    // uses to actually settle trades. Every user gets this exact number;
    // nothing downstream is allowed to randomize or override it.
    MarketSetting.find({}).select("symbol payoutPct").lean(),
  ]);

  const payoutMap = new Map<string, number>(marketSettings.map((m) => [m.symbol, m.payoutPct]));
  ...
  const markets: LiveMarket[] = pairs
    .map((s: any) => {
      ...
      // Always read payout from MarketSetting (DB) — the same field
      // trade.service.ts uses to settle trades — so every user sees the
      // exact payout they'll actually be paid, never a per-request guess.
      const payout = payoutMap.get(s.symbol) ?? (category === "Gold" ? 76 : 80);
      return {
        id: base.toLowerCase(),
        symbol: s.symbol,
        base,
    ...
    // Ranked by payout, highest first — same order for every user since
    // payout itself is now a single backend-driven value (see payoutSort).
    .sort(payoutSort);
